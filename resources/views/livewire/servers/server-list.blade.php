<div class="flex flex-col md:flex-row gap-6">
    ...
    <div class="w-full md:w-64 flex-shrink-0">
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
            <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Groups</h3>
            <nav class="space-y-1">
                <button wire:click="$set('selectedGroup', '')" 
                    class="w-full text-left px-3 py-2 text-sm font-medium rounded-md {{ $selectedGroup === '' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                    All Servers
                </button>
                @foreach($groups as $group)
                    <button wire:click="$set('selectedGroup', '{{ $group }}')" 
                        class="w-full text-left px-3 py-2 text-sm font-medium rounded-md {{ $selectedGroup === $group ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                ...
                @if($hasUngrouped)
                    <button wire:click="$set('selectedGroup', 'ungrouped_hidden_key')" 
                        class="w-full text-left px-3 py-2 text-sm font-medium rounded-md {{ $selectedGroup === 'ungrouped_hidden_key' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                        <span class="flex items-center">
                            <svg class="mr-2 h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex-1 space-y-4">
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                ...
                <x-secondary-button x-on:click="$dispatch('open-modal', 'server-import-modal')">
                    Import
                </x-secondary-button>
                <x-primary-button x-data="" x-on:click="Livewire.dispatch('create-server')">
                    Add Server
                </x-primary-button>
            </div>
        </div>
